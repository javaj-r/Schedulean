...
- `application`: Depends only on `domain`. Contains Port interfaces and Use Case implementations. No infrastructure imports.
- `adapter.out` (Driven): Implements `application.port.out`. Contains JPA Entities, Spring Data Repositories, ShedLock wrappers, NATS clients, Cron calculation implementations.
- `adapter.in` (Driving): Implements `application.port.in` or uses Use Cases. Contains REST controllers, NATS consumers, and the dynamic Scheduler Poller.
